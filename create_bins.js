...
(function(){
	...
	if(process.argv[2]==='install'){
		if(process.platform==='win32'){
			fs.writeFileSync(path.join(path_bin_root,'sshex.cmd'),fs.readFileSync(path.join(__dirname,'sshex.cmd')));
			fs.writeFileSync(path.join(path_bin_root,'sshex'),fs.readFileSync(path.join(__dirname,'sshex.sh')));
		}else{
